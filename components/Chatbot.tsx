...
export const Chatbot: React.FC = () => {
  ...
  // Check for completed but unreviewed tours
  useEffect(() => {
     if (user && isOpen && !hasAskedForReview.current) {
         const unreviewedBooking = bookings.find(b => 
             b.userId === user.id && 
             b.status === 'COMPLETED' &&
         ...
         if (unreviewedBooking) {
             ...
             if (tour) {
                 const tourName = tour.title[language];
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const responseText = await sendMessageToGemini(messages.concat(userMsg), userMsg.text, language);
  ...
  return (
    <>
      ...
      <button 
        ...
        className={`fixed bottom-6 right-6 z-40 bg-teal-800 text-white p-4 rounded-full shadow-lg hover:bg-teal-700 transition-all duration-300 transform hover:scale-105 ${isOpen ? 'hidden' : 'flex'}`}
      >
      ...
      {/* Chat Window */}
      <div className={`fixed bottom-6 right-6 z-50 w-full max-w-sm bg-white rounded-2xl shadow-2xl transition-all duration-300 transform border border-stone-200 flex flex-col ${isOpen ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0 pointer-events-none'}`} style={{ height: '500px' }}>
                    ...
                    <ReactMarkdown 
                      ...
                      components={{
                        a: ({ node, ...props }) => {
                          ...
                          if (href?.startsWith('/')) {
                            return <Link to={href} className="text-teal-600 hover:text-teal-800 underline font-semibold">{children}</Link>;
                          }
                          return <a target="_blank" rel="noopener noreferrer" className="text-teal-600 hover:text-teal-800 underline font-semibold" {...props}>{children}</a>;
                        },
                        ...
                        h1: ({ node, ...props }) => <h1 className="text-lg font-bold mb-2 text-teal-900" {...props} />,
                        h2: ({ node, ...props }) => <h2 className="text-base font-bold mb-2 text-teal-900" {...props} />,
                        h3: ({ node, ...props }) => <h3 className="text-sm font-bold mb-1 text-teal-900" {...props} />,
                        blockquote: ({ node, ...props }) => <blockquote className="border-l-4 border-teal-200 pl-3 italic text-stone-600 my-2" {...props} />,
                      }}
                    >
                      {msg.text}
                    </ReactMarkdown>
