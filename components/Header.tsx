...
export default function Header() {
    const { language, setLanguage, currency, setCurrency, user, authReady, logout, t } = useApp();
    ...
    // helper render auth block (desktop)
    ...
    // helper render auth block (mobile)
            ...
            {isMenuOpen && (
                <div className="md:hidden bg-sand-50 border-t border-sand-200 absolute w-full h-screen">
                    <div className="px-4 pt-4 pb-6 space-y-2 flex flex-col items-center text-lg">
                        <Link href="/" onClick={() => setIsMenuOpen(false)} className="block px-3 py-2 text-earth-900">
                            {t("nav.home")}
                        </Link>
                        <Link href="/tours" onClick={() => setIsMenuOpen(false)} className="block px-3 py-2 text-earth-900">
                            {t("nav.tours")}
                        </Link>
                        <Link href="/locations" onClick={() => setIsMenuOpen(false)} className="block px-3 py-2 text-earth-900">
                            {t("nav.locations")}
                        </Link>
                        <Link href="/courses" onClick={() => setIsMenuOpen(false)} className="block px-3 py-2 text-earth-900">
                            {t("nav.courses")}
                        </Link>
                        ...
                        <div className="flex space-x-6 pt-8 mt-4 w-full justify-center">
                            <button onClick={() => setLanguage(language === "vi" ? "en" : "vi")} className="uppercase text-sm font-bold text-stone-500">
                                {language}
                            </button>
                            <button onClick={() => setCurrency(currency === "VND" ? "USD" : "VND")} className="uppercase text-sm font-bold text-stone-500">
                                {currency}
                            </button>
                        </div>
                    </div>
