...
import { MapPin, ArrowRight, ArrowLeft, Tag } from 'lucide-react';

export const LocationDetail: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const { locations, tours, language, convertPrice } = useApp();
  const location = locations.find(l => l.id === id);

  // Find tours that include this location
  const relatedTours = tours.filter(t => t.locations.includes(id || ''));

  if (!location) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-sand-50">
        <div className="text-center">
           <h2 className="text-2xl font-bold text-earth-900 mb-4">{language === 'vi' ? 'Không tìm thấy địa điểm' : 'Location not found'}</h2>
           <Link to="/locations"><Button>{language === 'vi' ? 'Quay lại bản đồ' : 'Back to Locations'}</Button></Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-sand-50 pb-20">
       ...
       <div className="relative h-[60vh]">
          ...
          <div className="absolute bottom-0 left-0 w-full p-8 md:p-12 max-w-7xl mx-auto">
             <div className="mb-4">
                 <Link to="/locations" className="text-white/80 hover:text-white text-sm flex items-center mb-4 transition-colors">
                     <ArrowLeft className="w-4 h-4 mr-1" /> {language === 'vi' ? 'Tất cả địa điểm' : 'All Locations'}
                 </Link>
                 <span className="bg-primary text-white px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">{location.type}</span>
             </div>
       ...
       <div className="max-w-7xl mx-auto px-6 py-12">
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
               ...
               <div className="lg:col-span-2">
                   <h2 className="text-2xl font-serif font-bold text-earth-900 mb-6">{language === 'vi' ? 'Về địa điểm này' : 'About this place'}</h2>
                   <p className="text-lg text-stone-600 leading-relaxed mb-8">
                       {location.description 
                        ? location.description[language] 
                        : (language === 'vi' ? 'Mô tả đang được cập nhật.' : 'Description coming soon.')}
                   </p>
                   ...
                   {location.introduction && (
                       <div className="mb-8">
                           <h3 className="text-xl font-serif font-bold text-earth-900 mb-3">{language === 'vi' ? 'Giới thiệu chung' : 'Introduction'}</h3>
                   ...
                   {location.history && (
                       <div className="mb-8">
                           <h3 className="text-xl font-serif font-bold text-earth-900 mb-3">{language === 'vi' ? 'Lịch sử hình thành' : 'History'}</h3>
                   ...
                   {location.significance && (
                       <div className="mb-12 bg-sand-50 p-6 rounded-2xl border-l-4 border-amber-400">
                           <h3 className="text-lg font-bold text-earth-900 mb-2">{language === 'vi' ? 'Ý nghĩa văn hóa & Tâm linh' : 'Cultural & Spiritual Significance'}</h3>
                       ...
                       {relatedTours.length > 0 ? (
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                               {relatedTours.map(tour => (
                                   <Link to={`/tours/${tour.id}`} key={tour.id} className="group bg-white rounded-2xl p-4 shadow-sm hover:shadow-lg transition-all border border-sand-100 flex gap-4">
                                       <img src={tour.images[0]} alt="" className="w-24 h-24 rounded-xl object-cover shrink-0" />
                                       <div className="flex flex-col justify-between py-1">
                                           <div>
                                               <h3 className="font-bold text-earth-900 group-hover:text-primary line-clamp-2">{tour.title[language]}</h3>
                                               <p className="text-xs text-stone-500 mt-1">{tour.duration_days} {language === 'vi' ? 'Ngày' : 'Days'}</p>
                                           </div>
                           ...
                           <p className="text-stone-500 italic">{language === 'vi' ? 'Hiện chưa có tour nào đi qua địa điểm này.' : 'No tours currently listed for this location.'}</p>
                       )}
                   </div>
               </div>

               {/* Right Column: Mini Map or Images */}
               <div className="lg:col-span-1">
                   <div className="bg-white p-6 rounded-3xl shadow-sm border border-sand-100 sticky top-24">
                       ...
                       <div className="grid grid-cols-2 gap-2">
                           ...
                           {/* Mocks for extra images using the main one with different crops if real ones existed */}
