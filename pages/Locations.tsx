...
export const Locations: React.FC = () => {
  const { locations, language } = useApp();
    ...
    <div className="min-h-screen py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16 animate-slide-up">
          ...
          <div className="w-24 h-1 bg-primary-light mx-auto mt-6 rounded-full"></div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {locations.map((loc, index) => (
            ...
            >
              <Link to={`/locations/${loc.id}`} className="relative h-64 overflow-hidden block cursor-pointer">
                <img 
                  src={loc.image} 
              ...
              <div className="p-8 flex-1 flex flex-col">
                <div className="mb-4">
                   ...
                   <Link to={`/locations/${loc.id}`}>
                      <h3 className="text-2xl font-serif font-bold text-earth-900 mt-2 mb-2 hover:text-primary transition-colors">{loc.name[language]}</h3>
                   </Link>
                ...
                <div className="mt-auto pt-6 border-t border-sand-100 flex justify-between items-center">
                   <Link to={`/locations/${loc.id}`} className="text-stone-500 hover:text-primary text-sm font-medium flex items-center transition-colors">
                      <Eye className="w-4 h-4 mr-1" />
                      {language === 'vi' ? 'Xem thông tin' : 'View Info'}
                   </Link>
                   <Link to={`/tours?search=${loc.name[language]}`} className="text-primary hover:text-primary-dark font-medium flex items-center transition-colors text-sm">
