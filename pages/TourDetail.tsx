...
export const TourDetail: React.FC = () => {
  ...
  const mappedLocations = tour.locations.map(locId => LOCATIONS.find(l => l.id === locId)).filter(Boolean);
    ...
    <div className="bg-white min-h-screen pb-20">
      ...
      <div className="h-[50vh] relative">
         ...
         <div className="absolute bottom-0 left-0 w-full p-8 max-w-7xl mx-auto">
            ...
            <h1 className="text-4xl md:text-5xl font-serif font-bold text-white mb-2">{tour.title[language]}</h1>
            <p className="text-teal-100 text-lg max-w-2xl">{tour.description[language]}</p>
         </div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 bg-white rounded-t-3xl p-8 shadow-sm">
             ...
             <div className="flex flex-wrap gap-6 border-b border-stone-100 pb-8 mb-8">
                <div className="flex items-center text-stone-600">
                   <div className="p-2 bg-stone-100 rounded-full mr-3"><Calendar className="w-5 h-5 text-teal-800" /></div>
                   <div>
                      <p className="text-xs text-stone-400 uppercase">Duration</p>
                      <p className="font-semibold">{tour.duration_days} {language === 'vi' ? 'Ngày' : 'Days'}</p>
                   </div>
                </div>
                <div className="flex items-center text-stone-600">
                   <div className="p-2 bg-stone-100 rounded-full mr-3"><Users className="w-5 h-5 text-teal-800" /></div>
                   <div>
                      <p className="text-xs text-stone-400 uppercase">Suitable For</p>
                      <p className="font-semibold">{tour.suitable_for[language]}</p>
                   </div>
                </div>
             </div>
             ...
             <div className="mb-12 space-y-8 animate-fade-in">
                <div>
                   <h2 className="text-2xl font-serif font-bold text-teal-900 mb-4">{language === 'vi' ? 'Giới thiệu' : 'Introduction'}</h2>
                   <p className="text-stone-600 leading-relaxed text-lg">{tour.introduction[language]}</p>
                </div>
                ...
                <div className="bg-sand-50 p-6 rounded-2xl border-l-4 border-amber-400">
                   ...
                   <p className="text-stone-700 italic font-medium text-lg">"{tour.meaning[language]}"</p>
                </div>
             </div>

             {/* Itinerary */}
             <h2 className="text-2xl font-serif font-bold text-teal-900 mb-6">{language === 'vi' ? 'Lịch Trình & Điểm Đến' : 'Itinerary & Locations'}</h2>
                   ...
                   <div key={loc?.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                      ...
                      <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-stone-50 p-4 rounded-xl border border-stone-100 shadow-sm">
                         <div className="flex items-center justify-between space-x-2 mb-1">
                            <h3 className="font-bold text-stone-800">{loc?.name[language]}</h3>
                         </div>
                         <div className="text-stone-500 text-sm">{loc?.region} - <span className="text-teal-600 italic">{loc?.type}</span></div>
                      </div>
                   </div>
                ))}
             </div>
             ...
             <div className="grid grid-cols-2 gap-4">
                <img src={tour.images[0]} className="rounded-lg w-full h-48 object-cover" />
                {tour.images[1] && <img src={tour.images[1]} className="rounded-lg w-full h-48 object-cover" />}
                {!tour.images[1] && <div className="bg-stone-100 rounded-lg flex items-center justify-center text-stone-400">More coming soon</div>}
             </div>
          ...
          <div className="w-full lg:w-96">
             <div className="bg-white p-6 rounded-2xl shadow-xl sticky top-24 border border-stone-100">
                <div className="mb-6">
                   <p className="text-stone-500 text-sm">{language === 'vi' ? 'Giá từ' : 'Price starts from'}</p>
                ...
                <div className="space-y-4 mb-6">
                   <div className="flex items-start">
                      ...
                      <span className="text-sm text-stone-600">{language === 'vi' ? 'Hướng dẫn viên chuyên nghiệp' : 'Professional Zen Guide'}</span>
                   </div>
                   <div className="flex items-start">
                      <CheckCircle className="w-5 h-5 text-amber-500 mr-2 shrink-0 mt-0.5" />
                      <span className="text-sm text-stone-600">{language === 'vi' ? 'Bao gồm vé tham quan' : 'Entrance fees included'}</span>
                   </div>
                   <div className="flex items-start">
                      <CheckCircle className="w-5 h-5 text-amber-500 mr-2 shrink-0 mt-0.5" />
                      <span className="text-sm text-stone-600">{language === 'vi' ? 'Bữa ăn chay thanh tịnh' : 'Vegetarian meals'}</span>
                   </div>
                </div>

                <Link to={`/booking/${tour.id}`} className="block w-full">
                   <Button size="lg" className="w-full">{language === 'vi' ? 'Đặt Tour Ngay' : 'Book Now'}</Button>
                </Link>
