...
type Tour = {
    ...
    price_vnd: number;
    images: string[];
    locations: string[];     // list locationId
};
...
export default function ToursPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!res.ok) throw new Error(`API /api/tours failed: ${res.status}`);

                const data = await res.json();

                // API trả Tour[] trực tiếp
                setTours(Array.isArray(data) ? data : (data.tours ?? []));

                // locations hiện API chưa trả -> tạm để []
                setLocations([]);
            } catch (e: any) {
                if (!alive) return;
    ...
    const filteredTours = useMemo(() => {
        ...
        return tours.filter((tour) => {
            ...
            const title = (tour.title?.[language as Lang] ?? '').toLowerCase();
            const matchSearch = title.includes(q);
            return matchLevel && matchSearch;
        });
    }, [tours, filterLevel, searchQuery, language]);
                ...
                <div className="flex flex-col md:flex-row justify-between items-center mb-12 space-y-4 md:space-y-0">
                    <div>
                        ...
                        <p className="text-stone-500 mt-2">
                            {language === 'vi'
                                ? 'Chọn hành trình phù hợp với tâm hồn bạn.'
                                : 'Choose a journey that suits your soul.'}
                        </p>
                    </div>
                ...
                {!loading && !error && (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        {filteredTours.map((tour) => {
                            const region = locations.find((l) => l.id === tour.locations?.[0])?.region;
                            const hrefId = tour.slug ?? tour.id; // ✅ dùng slug nếu có, fallback id
                                ...
                                >
                                    <div className="w-full md:w-2/5 h-48 md:h-full relative overflow-hidden">
                                        <Image
                                            ...
                                            alt={tour.title?.[language as Lang] ?? 'Tour'}
                                            fill
                                    ...
                                    <div className="flex-1 p-6 flex flex-col justify-between">
                                        <div>
                                            <h3 className="text-xl font-serif font-bold text-teal-900 mb-2 group-hover:text-primary transition-colors">
                                                {tour.title?.[language as Lang]}
                                            </h3>
                                            <p className="text-stone-500 text-sm mb-4 line-clamp-2">
                                                {tour.description?.[language as Lang]}
                                            </p>
                      ...
                      <span className="text-lg font-bold text-teal-800">
                        ...
                        {/*<div className="text-xs text-red-500">tours: {tours.length}</div>*/}
