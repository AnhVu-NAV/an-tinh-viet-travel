...
export default function AdminDashboardClient() {
    ...
    const [tourModal, setTourModal] = useState<{ isOpen: boolean; mode: "ADD" | "EDIT"; data: Partial<Tour> }>({
        isOpen: false,
        mode: "ADD",
        data: {},
    });
    const [locationModal, setLocationModal] = useState<{ isOpen: boolean; mode: "ADD" | "EDIT"; data: Partial<Location> }>({
        isOpen: false,
        mode: "ADD",
        data: {},
    });
    const [discountModal, setDiscountModal] = useState<{ isOpen: boolean; mode: "ADD" | "EDIT"; data: Partial<Discount> }>({
        isOpen: false,
        mode: "ADD",
        data: {},
    });
    const [courseModal, setCourseModal] = useState<{ isOpen: boolean; mode: "ADD" | "EDIT"; data: Partial<Course> }>({
        isOpen: false,
        mode: "ADD",
        data: {},
    });

    const openDeleteModal = (type: typeof deleteModal.type, id: string) => setDeleteModal({ isOpen: true, type, id });
    ...
    const toggleLocationForTour = (locationId: string) => {
        ...
        const next = current.includes(locationId) ? current.filter((x) => x !== locationId) : [...current, locationId];
    ...
    const toggleUserActive = async (id: string) => {
        ...
        const res = await fetch(`/api/admin/users/${encodeURIComponent(id)}/toggle-active`, { method: "PATCH" });
    ...
    const handleSaveLocation = async (e: React.FormEvent) => {
        ...
        if (locationModal.mode === "ADD") {
            ...
            const res = await fetch(`/api/admin/locations/${encodeURIComponent(String(data.id))}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    name: data.name,
    ...
    const handleSaveCourse = async (e: React.FormEvent) => {
        ...
        if (courseModal.mode === "ADD") {
            ...
            const res = await fetch(`/api/admin/courses/${encodeURIComponent(String(data.id))}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    title: data.title,
    ...
    const confirmDelete = async () => {
        ...
        if (type === "LOCATION") {
            ...
            const res = await fetch(`/api/admin/locations/${encodeURIComponent(id)}`, { method: "DELETE" });
            if (!res.ok) await refetchBootstrap();
        }
        ...
        if (type === "DISCOUNT") {
            ...
            const res = await fetch(`/api/admin/discounts/${encodeURIComponent(id)}`, { method: "DELETE" });
            if (!res.ok) await refetchBootstrap();
        }
    ...
    const handleSaveDiscount = async (e: React.FormEvent) => {
        ...
        if (discountModal.mode === "ADD") {
            ...
            const res = await fetch(`/api/admin/discounts/${encodeURIComponent(String(data.code))}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    percent: Number(data.percent) || 10,
    ...
    const filteredTours = useMemo(
        () => tours.filter((t) => t.title.en.toLowerCase().includes(searchTerm.toLowerCase())),
        [tours, searchTerm]
    );
    const filteredLocations = useMemo(
        () => locations.filter((l) => l.name.en.toLowerCase().includes(searchTerm.toLowerCase())),
        [locations, searchTerm]
    );
    const filteredBookings = useMemo(
        () => bookings.filter((b) => b.id.includes(searchTerm) || b.status.toLowerCase().includes(searchTerm.toLowerCase())),
        [bookings, searchTerm]
    );
    const filteredDiscounts = useMemo(
        () => discounts.filter((d) => d.code.toLowerCase().includes(searchTerm.toLowerCase())),
    ...
    const filteredCourses = useMemo(
        () => courses.filter((c) => c.title.en.toLowerCase().includes(searchTerm.toLowerCase())),
        [courses, searchTerm]
    );
    ...
    const SidebarItem = ({ id, icon: Icon, label }: { id: Tab; icon: any; label: string }) => (
        <button
            ...
            className={`w-full flex items-center px-4 py-3 rounded-xl text-sm font-medium transition-colors mb-1 ${
                activeTab === id ? "bg-primary/10 text-primary" : "text-stone-600 hover:bg-sand-100"
            }`}
        >
        ...
        <div className="min-h-screen bg-sand-50 flex font-sans text-earth-900">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            <div className="bg-white p-6 rounded-3xl shadow-sm border border-sand-200 hover:shadow-md transition-shadow">
                                ...
                                <div className="text-3xl font-bold text-earth-900 mt-1">
                                    {convertPrice(bookings.reduce((acc, b) => acc + (b.status !== "CANCELLED" ? b.totalPrice : 0), 0))}
                                </div>
                            </div>
                    ...
                    <div className="bg-white rounded-3xl shadow-sm border border-sand-200 overflow-hidden animate-slide-up">
                      ...
                      <span className="px-2.5 py-1 bg-primary/10 text-primary rounded-full text-xs font-bold uppercase tracking-wide">
                        {tour.level}
                                    ...
                                    {isSuperAdmin && (
                                        <td className="p-5 flex justify-end gap-2">
                                            <Button variant="ghost" size="sm" onClick={() => setTourModal({ isOpen: true, mode: "EDIT", data: tour })}>
                                                <Edit className="w-4 h-4 text-stone-500" />
                                            </Button>
                                            <Button variant="ghost" size="sm" onClick={() => openDeleteModal("TOUR", tour.id)} className="hover:bg-red-50">
                        ...
                        {filteredTours.length === 0 && (
                            <div className="p-10 text-center text-stone-400 italic">No tours found matching your search.</div>
                        )}
                    </div>
                )}
                            ...
                            <div key={loc.id} className="bg-white p-5 rounded-3xl shadow-sm hover:shadow-md transition-all border border-sand-200 flex flex-col group">
                                <div className="flex gap-4 mb-4">
                                    <div className="w-20 h-20 rounded-2xl overflow-hidden relative shrink-0">
                                        <Image src={loc.image ?? "/placeholder.jpg"} alt="" fill sizes="80px" className="object-cover group-hover:scale-105 transition-transform" />
                                    </div>
                                    <div>
                      ...
                      {loc.type}
                                ...
                                {isSuperAdmin && (
                                    <div className="mt-auto pt-4 border-t border-sand-100 flex justify-end gap-2">
                                        <Button variant="ghost" size="sm" onClick={() => setLocationModal({ isOpen: true, mode: "EDIT", data: loc })} className="text-stone-500 hover:text-primary hover:bg-primary/5">
                                            Edit
                                        </Button>
                                        <Button variant="ghost" size="sm" onClick={() => openDeleteModal("LOCATION", loc.id)} className="text-red-400 hover:text-red-600 hover:bg-red-50">
                                            Delete
                                        </Button>
                                    </div>
                                )}
                        ...
                        <table className="w-full text-left">
                            ...
                            {filteredCourses.map((course) => (
                                <tr key={course.id} className="hover:bg-sand-50/50">
                                    ...
                                    {isSuperAdmin && (
                                        <td className="p-5 flex justify-end gap-2">
                                            <Button variant="ghost" size="sm" onClick={() => setCourseModal({ isOpen: true, mode: "EDIT", data: course })}>
                                                <Edit className="w-4 h-4 text-stone-500" />
                                            </Button>
                                            <Button variant="ghost" size="sm" onClick={() => openDeleteModal("COURSE", course.id)} className="hover:bg-red-50">
                                                <Trash2 className="w-4 h-4 text-red-400" />
                                            </Button>
                                        </td>
                                    )}
                                ...
                                <tr key={bk.id} className="hover:bg-sand-50/50">
                                    ...
                                    <td className="p-5">
                                        <p className="font-bold text-sm text-earth-900">
                                            {tours.find((t) => t.id === bk.tourId)?.title.en || "Unknown Tour"}
                                        </p>
                                ...
                                <tr key={d.code} className="hover:bg-sand-50/50">
                                    ...
                                    <td className="p-5 text-sm">
                                        <div className="w-32 bg-sand-200 rounded-full h-1.5 mb-2 overflow-hidden">
                                            <div className="bg-primary h-full rounded-full" style={{ width: `${(d.used_count / d.usage_limit) * 100}%` }} />
                                        </div>
                                    ...
                                    <td className="p-5 flex justify-end gap-2">
                                        <Button variant="ghost" size="sm" onClick={() => setDiscountModal({ isOpen: true, mode: "EDIT", data: d })}>
                                            <Edit className="w-4 h-4 text-stone-500" />
                                        </Button>
                                        <Button variant="ghost" size="sm" onClick={() => openDeleteModal("DISCOUNT", d.code)} className="hover:bg-red-50">
                                            <Trash2 className="w-4 h-4 text-red-400" />
                                        </Button>
                                    </td>
                                </tr>
            ...
            >
                <div className="text-center py-6">
                    ...
                    <div className="flex gap-4 justify-center">
                        <Button variant="outline" className="border-stone-200 text-stone-600" onClick={() => setDeleteModal((p) => ({ ...p, isOpen: false }))}>
                            Cancel
                        </Button>
            ...
            >
                <form onSubmit={handleSaveTour} className="space-y-6">
                    <div className="grid grid-cols-2 gap-6">
                        <div className="col-span-2 md:col-span-1">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setTourModal((p) => ({ ...p, data: { ...p.data, title: { ...(p.data.title || { en: "", vi: "" }), en: e.target.value } } }))
                                }
                            />
                        </div>
                        <div className="col-span-2 md:col-span-1">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setTourModal((p) => ({ ...p, data: { ...p.data, title: { ...(p.data.title || { en: "", vi: "" }), vi: e.target.value } } }))
                                }
                            />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setTourModal((p) => ({ ...p, data: { ...p.data, price_vnd: parseInt(e.target.value || "0", 10) } }))}
                            />
                        </div>
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setTourModal((p) => ({ ...p, data: { ...p.data, duration_days: parseInt(e.target.value || "1", 10) } }))}
                            />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            {(["light", "moderate", "deep"] as const).map((level) => (
                                <div
                                    key={level}
                                    onClick={() => setTourModal((p) => ({ ...p, data: { ...p.data, level } }))}
                                        ...
                                        <div key={idx} className="relative group w-20 h-20 flex-shrink-0">
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    setTourModal((p) => ({ ...p, data: { ...p.data, images: newImages } }));
                                                }}
                                ...
                                <input
                                    ...
                                    onChange={async (e) => {
                                        ...
                                        try {
                                            ...
                                            setTourModal((p) => ({ ...p, data: { ...p.data, images: [...current, driveUrl] } }));
                                        } catch (err) {
                                            console.error(err);
                                ...
                                onKeyDown={(e) => {
                                    if (e.key === "Enter") {
                                        ...
                                        if (url) {
                                            ...
                                            const current = tourModal.data.images || [];
                                            setTourModal((p) => ({ ...p, data: { ...p.data, images: [...current, processed] } }));
                            ...
                            {locations.map((loc) => (
                                <div
                                    ...
                                    className={`p-3 rounded-lg border cursor-pointer flex items-center gap-3 transition-all ${
                                        tourModal.data.locations?.includes(loc.id) ? "border-primary bg-primary/10" : "border-transparent hover:bg-white"
                                    }`}
                                >
                                    <div className={`w-5 h-5 rounded border flex items-center justify-center ${
                                        tourModal.data.locations?.includes(loc.id) ? "bg-primary border-primary text-white" : "border-stone-300 bg-white"
                                    }`}>
                    ...
                    <div className="pt-6 flex justify-end gap-3 border-t border-sand-100">
                        <Button type="button" variant="ghost" onClick={() => setTourModal((p) => ({ ...p, isOpen: false }))}>
                            Cancel
                        </Button>
            ...
            >
                <form onSubmit={handleSaveLocation} className="space-y-6">
                    <div className="grid grid-cols-2 gap-6">
                        <div className="col-span-2 md:col-span-1">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setLocationModal((p) => ({ ...p, data: { ...p.data, name: { ...(p.data.name || { en: "", vi: "" }), en: e.target.value } } }))
                                }
                            />
                        </div>
                        <div className="col-span-2 md:col-span-1">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setLocationModal((p) => ({ ...p, data: { ...p.data, name: { ...(p.data.name || { en: "", vi: "" }), vi: e.target.value } } }))
                                }
                            />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setLocationModal((p) => ({ ...p, data: { ...p.data, region: e.target.value } }))}
                            />
                        </div>
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setLocationModal((p) => ({ ...p, data: { ...p.data, type: e.target.value } }))}
                            />
                        </div>
                    </div>
                        ...
                        <div className="flex items-center gap-4">
                            {locationModal.data.image && (
                                <div className="w-24 h-24 rounded-lg overflow-hidden border border-sand-200 shrink-0 relative group">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setLocationModal((p) => ({ ...p, data: { ...p.data, image: "" } }))}
                            ...
                            <div className="flex-1 space-y-2">
                                <label className="cursor-pointer inline-flex items-center gap-2 px-4 py-2 bg-sand-100 hover:bg-sand-200 text-stone-600 rounded-lg text-sm font-bold transition-colors">
                                    ...
                                    <input
                                        ...
                                        accept="image/*"
                                        className="hidden"
                                        onChange={(e) => handleImageUpload(e, (url) => setLocationModal((p) => ({ ...p, data: { ...p.data, image: url } })))}
                                    />
                                </label>
                                ...
                                <input
                                    ...
                                    onChange={(e) =>
                                        setLocationModal((p) => ({ ...p, data: { ...p.data, image: processImageUrl(e.target.value) } }))
                                    }
                                />
                            </div>
                        </div>
                    ...
                    <div className="pt-6 flex justify-end gap-3 border-t border-sand-100">
                        <Button type="button" variant="ghost" onClick={() => setLocationModal((p) => ({ ...p, isOpen: false }))}>
                            Cancel
                        </Button>
                        <Button type="submit" className="px-8">
                            Save Location
            ...
            <Modal
                ...
                title={discountModal.mode === "ADD" ? "Create Discount Code" : "Edit Discount"}
            >
                <form onSubmit={handleSaveDiscount} className="space-y-6">
                    <div>
                        ...
                        <input
                            ...
                            onChange={(e) => setDiscountModal((p) => ({ ...p, data: { ...p.data, code: e.target.value } }))}
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setDiscountModal((p) => ({ ...p, data: { ...p.data, percent: parseInt(e.target.value || "0", 10) } }))}
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setDiscountModal((p) => ({ ...p, data: { ...p.data, usage_limit: parseInt(e.target.value || "0", 10) } }))}
                            />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            onChange={(e) => setDiscountModal((p) => ({ ...p, data: { ...p.data, valid_until: e.target.value } }))}
                        />
                    </div>

                    <div className="pt-6 flex justify-end gap-3 border-t border-sand-100">
                        <Button type="button" variant="ghost" onClick={() => setDiscountModal((p) => ({ ...p, isOpen: false }))}>
                            Cancel
                        </Button>
                        <Button type="submit" className="px-8">
                            Save Discount
                ...
                <form onSubmit={handleSaveCourse} className="space-y-6">
                    <div className="grid grid-cols-2 gap-6">
                        <div className="col-span-2 md:col-span-1">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setCourseModal((p) => ({ ...p, data: { ...p.data, title: { ...(p.data.title || { en: "", vi: "" }), en: e.target.value } } }))
                                }
                            />
                        </div>
                        ...
                        <div className="col-span-2 md:col-span-1">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setCourseModal((p) => ({ ...p, data: { ...p.data, title: { ...(p.data.title || { en: "", vi: "" }), vi: e.target.value } } }))
                                }
                            />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setCourseModal((p) => ({ ...p, data: { ...p.data, price_vnd: parseInt(e.target.value || "0", 10) } }))}
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setCourseModal((p) => ({ ...p, data: { ...p.data, duration: e.target.value } }))}
                    ...
                    <div>
                        ...
                        <input
                            ...
                            onChange={(e) => setCourseModal((p) => ({ ...p, data: { ...p.data, group_link: e.target.value } }))}
                            placeholder="https://..."
                        />
                    </div>
                        ...
                        <div className="flex items-center gap-4">
                            {courseModal.data.image && (
                                <div className="w-24 h-24 rounded-lg overflow-hidden border border-sand-200 shrink-0 relative group">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setCourseModal((p) => ({ ...p, data: { ...p.data, image: "" } }))}
                                        className="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity"
                                    >
                                        <X className="w-3 h-3" />
                                    </button>
                            ...
                            <div className="flex-1 space-y-2">
                                <label className="cursor-pointer inline-flex items-center gap-2 px-4 py-2 bg-sand-100 hover:bg-sand-200 text-stone-600 rounded-lg text-sm font-bold transition-colors">
                                    ...
                                    <input
                                        ...
                                        accept="image/*"
                                        className="hidden"
                                        onChange={(e) => handleImageUpload(e, (url) => setCourseModal((p) => ({ ...p, data: { ...p.data, image: url } })))}
                                    />
                                </label>
                                ...
                                <input
                                    ...
                                    onChange={(e) => setCourseModal((p) => ({ ...p, data: { ...p.data, image: processImageUrl(e.target.value) } }))}
                                />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="pt-6 flex justify-end gap-3 border-t border-sand-100">
                        <Button type="button" variant="ghost" onClick={() => setCourseModal((p) => ({ ...p, isOpen: false }))}>
                            Cancel
                        </Button>
                        <Button type="submit" className="px-8">
                            Save Course
